      ...
      <FsPage className="flex max-w-[100vw] flex-col overflow-x-hidden bg-fs-surface !px-2 !pt-2 min-[900px]:!px-4 min-[900px]:!pt-3 sm:!px-3">
        {dashQ.isPending && !dashQ.data ? (
          <div className="flex flex-1 flex-col gap-3 py-8" aria-busy="true" aria-label="Chargement du tableau de bord">
            ...
            <div className="h-40 w-full animate-pulse rounded-[8px] bg-neutral-200/80" />
            <div className="grid grid-cols-2 gap-2 min-[700px]:grid-cols-4">
              {Array.from({ length: 8 }).map((_, i) => (
                <div key={i} className="h-28 animate-pulse rounded-[8px] bg-neutral-200/70" />
              ))}
            </div>
          </div>
        ) : dashQ.isError ? (
  ...
  return (
    <FsPage className="flex flex-col min-[900px]:pt-6">
      ...
      <section
        ...
        className="rounded-md border border-black/[0.06] bg-fs-card p-3 shadow-sm sm:rounded-lg sm:p-4"
      >
        ...
        <div className="mt-3 flex flex-col gap-2 min-[900px]:flex-row min-[900px]:items-center min-[900px]:justify-between">
          <div className="flex flex-wrap items-stretch gap-2 min-[900px]:items-center">
            <button
              ...
              className={cn(
                "inline-flex min-h-10 items-center gap-1.5 rounded-sm border px-3 py-1.5 text-sm font-semibold transition-colors",
                scope === "company"
            ...
            {stores.length > 0 ? (
              ...
              <button
                ...
                className="inline-flex min-h-10 cursor-not-allowed items-center gap-1.5 rounded-sm border border-black/[0.06] bg-neutral-100 px-3 py-1.5 text-sm font-semibold text-neutral-400"
              >
            ...
            {scope === "store" && stores.length > 1 ? (
              <label className="flex min-w-0 max-w-[220px] min-[900px]:w-[220px] items-center gap-2">
                ...
                <select
                  ...
                  className="w-full min-w-0 rounded-sm border border-black/[0.12] bg-fs-card px-2 py-2 text-sm"
                >
                  {stores.map((s) => (
          ...
          <div className="flex flex-wrap items-stretch gap-2 min-[900px]:items-center min-[900px]:justify-end">
            {(
              ...
              <button
                ...
                onClick={() => setPeriod(p)}
                className={cn(
                  "min-h-10 rounded-sm px-3 py-1.5 text-sm font-semibold",
                  period === p
      ...
      {dashQ.isPending && !dashQ.data ? (
        ...
        <div className="mt-3 min-[900px]:mt-5 rounded-md border border-red-200 bg-red-50 p-4 text-sm text-red-800">
          {(dashQ.error as Error)?.message ?? "Impossible de charger le tableau de bord."}
        </div>
      ) : d ? (
        <>
          {/* Stats du jour */}
          <section className="mt-3 min-[900px]:mt-5 rounded-md border-2 border-[color-mix(in_srgb,var(--fs-accent)_35%,transparent)] bg-fs-card shadow-sm">
              ...
              <div className="flex flex-col gap-3 min-[380px]:flex-row min-[380px]:items-center min-[380px]:justify-between">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="min-w-0 flex-1 rounded-sm border border-black/[0.12] px-2 py-2 text-sm"
                  />
                  <button
                    type="button"
                    onClick={() =>
                    ...
                    className="shrink-0 rounded-sm border border-black/[0.1] px-3 py-2 text-sm font-medium text-[var(--fs-accent)]"
                  >
                    Aujourd&apos;hui
                  </button>
                </div>
...
function DayStat({
  ...
  return (
    <div
      className={cn(
        "rounded-sm bg-neutral-100/80 px-3 py-2.5 min-[600px]:px-3 min-[600px]:py-3",
        className,
      )}
    >
...
function KpiCard({
  ...
  const inner = (
    <>
      <div className="flex items-start justify-between gap-2">
        ...
        <span
          className="rounded-sm p-1.5 min-[600px]:p-2"
  ...
  if (inventoryLink) {
    return (
      <Link
        href={inventoryLink}
        className={cn(
          "block rounded-md border bg-fs-card p-3 shadow-sm min-[600px]:p-4",
          accent
  ...
  return (
    <div
      className={cn(
        "rounded-md border bg-fs-card p-3 shadow-sm min-[600px]:p-4",
        accent
          ? "border-[color-mix(in_srgb,var(--fs-accent)_40%,transparent)]"
          : "border-black/[0.06]",
      )}
...
function ChartCard({
  ...
  return (
    <section
      className={cn(
        "rounded-md border border-black/[0.06] bg-fs-card shadow-sm",
        className,
      )}
    >
...
function Shortcuts({
  ...
  return (
    <section
      className={cn(
        "shrink-0 rounded-md border border-black/[0.06] bg-fs-card p-3 shadow-sm min-[900px]:p-4",
        className,
      )}
    >
      ...
      <div className="mt-3 grid grid-cols-2 gap-2.5 min-[560px]:grid-cols-3 min-[780px]:grid-cols-4 min-[1000px]:grid-cols-5">
        {tiles.map((t) => (
          <Link
            key={t.label}
            href={t.href}
            className="flex min-h-[52px] items-center gap-3 rounded-md border border-black/[0.08] px-3 py-2.5 transition-colors active:bg-neutral-50"
          >
            <span
