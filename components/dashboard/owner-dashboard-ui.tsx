      ...
      <div className="min-[900px]:px-1">
        ...
        <SoftCard className="mt-5 p-4 min-[900px]:p-5">
          <p className="text-sm font-semibold text-neutral-800">
            Bonjour ! <span aria-hidden>👋</span> Vos chiffres d’aujourd’hui
          </p>
                ...
                return (
                  <div className="mt-2.5 rounded-md border border-black/[0.06] bg-fs-surface-container/60 p-3 dark:border-white/10">
                    {/*
                      ...
                      la carte : des `inline-flex` sans `min-w-0` refusent de se
                      réduire sous leur contenu. Sur téléphone chaque légende prend sa
                      ligne, libellé à gauche et montant à droite ; dès 600 px on
                      retrouve la rangée d'origine, cette fois incapable de déborder.
                    */}
                    <div className="flex flex-col gap-1.5 text-xs min-[600px]:flex-row min-[600px]:items-center min-[600px]:justify-between min-[600px]:gap-3">
                      <span className="flex min-w-0 items-center gap-1.5 font-semibold text-emerald-700 dark:text-emerald-400">
                        <span className="h-2.5 w-2.5 shrink-0 rounded-full bg-emerald-500" aria-hidden />
                        <span className="min-w-0 truncate">Ventes du jour</span>
                        <span className="ml-auto shrink-0 tabular-nums min-[600px]:ml-0">
                          {formatCurrency(sales)}
                        </span>
                      </span>
                      <span className="flex min-w-0 items-center gap-1.5 font-semibold text-amber-700 dark:text-amber-400">
                        <span className="h-2.5 w-2.5 shrink-0 rounded-full bg-amber-500" aria-hidden />
                        <span className="min-w-0 truncate">Crédits remboursés</span>
                        <span className="ml-auto shrink-0 tabular-nums min-[600px]:ml-0">
                    ...
                    <p className="mt-1.5 text-[11px] leading-snug text-neutral-500">
                      Sur {formatCurrency(total)} encaissés {daySuffix},{" "}
                      {formatCurrency(credit)}{" "}
                      proviennent de crédits d&apos;anciennes ventes — votre vente réelle
                      du jour est de{" "}
