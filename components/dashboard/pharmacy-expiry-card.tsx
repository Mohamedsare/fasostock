...
export function PharmacyExpiryCard({
  ...
  return (
    <section className="mb-3 rounded-md border border-black/[0.06] bg-fs-card p-3 shadow-sm sm:rounded-lg sm:p-4 min-[900px]:mb-5">
            ...
            <ul className="mt-3 space-y-1.5">
              {summary!.items.map((it, i) => {
                ...
                return (
                  <li
                    key={`${it.productId}-${i}`}
                    className="flex items-center gap-2 rounded-sm bg-fs-surface-container px-2.5 py-1.5"
                  >
                    <MdWarningAmber
                      className={cn(
                        "h-4 w-4 shrink-0",
                    ...
                    <span
                      className={cn(
                        "shrink-0 rounded-xs px-1.5 py-0.5 text-[10px] font-semibold",
                        tone,
                      )}
                    >
