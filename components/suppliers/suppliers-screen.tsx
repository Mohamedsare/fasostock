  ...
  function exportExcel() {
    ...
    void (async () => {
      try {
        ...
        await downloadProSpreadsheet(`fournisseurs-${d}.xlsx`, "Fournisseurs", headers, matrix, {
          title: "FasoStock — Fournisseurs",
          subtitle: `${rows.length} fournisseur(s) · ${d}`,
        });
          ...
          <div className="mt-4 flex flex-col gap-2">
            {rows.length > 0 ? (
              <button
                type="button"
                onClick={exportExcel}
                className="flex w-full items-center justify-center gap-2 rounded-[10px] border border-black/[0.12] bg-fs-card px-4 py-3 text-sm font-semibold text-neutral-800 shadow-sm active:scale-[0.99]"
              >
          ...
          <div className="flex shrink-0 flex-wrap items-center justify-end gap-2">
            {rows.length > 0 ? (
              <button
                type="button"
                onClick={exportExcel}
                className="inline-flex items-center gap-2 rounded-[10px] border border-black/[0.12] bg-fs-card px-4 py-2.5 text-sm font-semibold text-neutral-800 shadow-sm active:scale-[0.99]"
              >
                <MdDownload className="h-5 w-5 shrink-0" aria-hidden />
                Exporter Excel
              </button>
