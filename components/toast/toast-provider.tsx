...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      ...
      {active ? (
        <div
          className="pointer-events-none fixed inset-x-0 z-[100] flex justify-center px-4 max-md:bottom-[calc(5rem+var(--fs-safe-bottom))] md:bottom-8"
          aria-live={live}
          role={active.type === "error" ? "alert" : "status"}
        >
          <div
            className={cn(
              "pointer-events-auto flex w-full max-w-lg items-start gap-3 rounded-xl px-4 py-3.5 text-white shadow-lg transition-[opacity,transform] duration-200 ease-out",
