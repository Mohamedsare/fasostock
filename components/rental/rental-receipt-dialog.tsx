...
import { RentalReceiptPreview } from "@/components/rental/rental-receipt-preview";
import {
  fetchRentalReceiptData,
  generateRentalReceiptPdfBlob,
} from "@/lib/features/rental/ticket-api";
 ...
 * La largeur par défaut suit le réglage de la boutique (page Boutiques → Caisse
 * rapide), modifiable ici à la volée.
 */
export function RentalReceiptDialog({
  paymentId,
  onClose,
}: {
  paymentId: string;
  ...
  const q = useQuery({
    queryKey: ["rental-receipt", paymentId],
    queryFn: () => fetchRentalReceiptData(paymentId),
    staleTime: 60_000,
  });
  ...
  async function withBlob(action: (blob: Blob) => void, mode: "print" | "download") {
    ...
    setBusy(mode);
    try {
      const blob = await generateRentalReceiptPdfBlob(paymentId, {
        paperWidthMm: effectiveWidth,
      });
      action(blob);
    } catch (e) {
      toast.error(messageFromUnknownError(e, "Impossible de générer la quittance."));
    } finally {
      setBusy(null);
    }
  }
  ...
  function handlePrint() {
    toast.info("Préparation de la quittance…");
    void withBlob((blob) => {
      printInvoicePdf(blob);
      window.setTimeout(() => toast.success("Quittance envoyée à l'imprimante."), 400);
    }, "print");
  }
  ...
  function handleDownload() {
    void withBlob((blob) => {
      ...
      a.href = url;
      a.download = `quittance-${q.data?.receiptNumber ?? "location"}.pdf`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      window.setTimeout(() => URL.revokeObjectURL(url), 2000);
      toast.success("Quittance enregistrée.");
    }, "download");
  }
      ...
      >
        <div className="flex shrink-0 items-center justify-between border-b border-black/[0.07] px-4 py-3 dark:border-white/10">
          <div className="min-w-0">
            <h2 className="truncate text-base font-bold text-fs-text">Quittance locataire</h2>
            <p className="truncate text-xs text-neutral-500">
              {q.data ? `${q.data.receiptNumber} · ${q.data.tenantName}` : "Chargement…"}
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="min-h-0 flex-1 overflow-y-auto bg-neutral-100/60 px-4 py-4 dark:bg-black/20">
          {q.isLoading ? (
            ...
            <p className="py-10 text-center text-sm font-medium text-red-600">
              {messageFromUnknownError(q.error, "Quittance introuvable.")}
            </p>
          ) : (
            <RentalReceiptPreview data={q.data} />
          )}
        </div>
