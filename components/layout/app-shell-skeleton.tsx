  ...
  return (
    <div className="flex min-h-dvh flex-col bg-fs-surface text-fs-text">
      <div className="flex min-h-0 flex-1 overflow-hidden">
        <aside
          className={cn(
            "hidden h-full min-h-0 w-[260px] shrink-0 flex-col border-r border-black/[0.06] bg-fs-card dark:border-white/[0.08] lg:flex",
        ...
        >
          ...
          >
            <div className="flex items-center gap-3">
              <div className={cn("h-9 w-9 shrink-0 rounded-xl", pulse)} />
              <div className={cn("h-5 w-28 rounded-lg", pulse)} />
            </div>
          </div>
          <div className="flex flex-1 flex-col gap-2 overflow-hidden p-2.5">
            {Array.from({ length: 9 }).map((_, i) => (
              <div key={i} className="flex items-center gap-3 rounded-2xl px-3 py-2">
                <div className={cn("h-9 w-9 shrink-0", pulse)} />
